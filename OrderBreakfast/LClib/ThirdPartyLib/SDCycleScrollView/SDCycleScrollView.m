...
- (void)setTitlesAttributedGroup:(NSArray *)titlesAttributedGroup
{
    _titlesAttributedGroup = titlesAttributedGroup;
    if (self.onlyDisplayText) {
        NSMutableArray *temp = [NSMutableArray new];
        for (int i = 0; i < _titlesAttributedGroup.count; i++) {
            [temp addObject:@""];
        }
        self.backgroundColor = [UIColor clearColor];
        self.imageURLStringsGroup = [temp copy];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_titlesAttributedGroup.count && itemIndex < _titlesAttributedGroup.count) {
        cell.attributedTitle = _titlesAttributedGroup[itemIndex];
    }
